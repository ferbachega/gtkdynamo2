...
class ADGridMap:
    ...
    def write(self, fp):
        ...
        print >>fp, 'NELEMENTS %d %d %d' % (
            self.npts[0], self.npts[1], self.npts[2])
        print >>fp, 'CENTER %5.3f %5.3f %5.3f' % (
            self.center[0], self.center[1], self.center[2])
    ...
    def writeDX(self, fname):
        ...
        print >>fp, '#=================================='
        print >>fp, 'object 1 class gridpositions counts %d %d %d' % (
            nx, ny, nz)
        ...
        print >>fp, 'object 2 class gridconnections counts %d %d %d' % (
            nx, ny, nz)
        print >>fp, 'object 3 class array type double rank 0 items %d data follows' % len(
            vals)
        for k in range(nz):
            col = 0
            for j in range(ny):
...
class Autodock:
    ...
    def __init__(self, app):
        ...
        self.box_display_cylinder_size.set(
            default_settings['box_cylinder_size'])
        self.box_display_line_width = DoubleVar()
        self.box_display_line_width.set(
            default_settings['box_mesh_line_width'])
        ...
        self.grid_page_main_group = Pmw.Group(
            self.grid_definition_page, tag_text='Grid Definition')
        self.grid_page_main_group.pack(fill='both', expand=0, padx=10, pady=5)

        # grid parameters on the left
        self.grid_page_left_side = Pmw.Group(
            self.grid_page_main_group.interior(), tag_text='Parameters')
        self.grid_page_left_side.pack(
            side=LEFT, fill='both', expand=0, padx=10, pady=3)

        # grid spacing entry
        self.grid_spacing_frame = Tkinter.Frame(
            self.grid_page_left_side.interior())
        self.grid_spacing_label = Label(
            self.grid_spacing_frame, text='Spacing:', width=10)
        self.grid_spacing_location = Entry(
            self.grid_spacing_frame, textvariable=self.grid_spacing, bg='black', fg='green', width=10)
        self.grid_spacing_scrollbar = Scrollbar(
            self.grid_spacing_frame, orient='horizontal', command=self.grid_spacing_changed)
        ...
        self.n_points_X_frame = Tkinter.Frame(
            self.grid_page_left_side.interior())
        self.n_points_X_label = Label(
            self.n_points_X_frame, text='X-points:', width=10)
        self.n_points_X_location = Entry(
            self.n_points_X_frame, textvariable=self.n_points_X, bg='black', fg='green', width=10)
        self.n_points_X_scrollbar = Scrollbar(
            self.n_points_X_frame, orient='horizontal', command=self.n_points_X_changed)
        ...
        self.n_points_Y_frame = Tkinter.Frame(
            self.grid_page_left_side.interior())
        self.n_points_Y_label = Label(
            self.n_points_Y_frame, text='Y-points:', width=10)
        self.n_points_Y_location = Entry(
            self.n_points_Y_frame, textvariable=self.n_points_Y, bg='black', fg='green', width=10)
        self.n_points_Y_scrollbar = Scrollbar(
            self.n_points_Y_frame, orient='horizontal', command=self.n_points_Y_changed)
        ...
        self.n_points_Z_frame = Tkinter.Frame(
            self.grid_page_left_side.interior())
        self.n_points_Z_label = Label(
            self.n_points_Z_frame, text='Z-points:', width=10)
        self.n_points_Z_location = Entry(
            self.n_points_Z_frame, textvariable=self.n_points_Z, bg='black', fg='green', width=10)
        self.n_points_Z_scrollbar = Scrollbar(
            self.n_points_Z_frame, orient='horizontal', command=self.n_points_Z_changed)
        ...
        self.display_button_box = Pmw.ButtonBox(
            self.grid_page_main_group.interior(), padx=0, pady=1, orient='vertical')
        ...
        self.display_button_box.add(
            'Change Box Color', command=self.change_box_color)

        # display options on the right
        self.grid_page_right_side = Pmw.Group(
            self.grid_page_main_group.interior(), tag_text='Display Options')
        self.grid_page_right_side.pack(
            side=LEFT, fill='both', expand=0, padx=10, pady=3)

        self.box_display_radiogroups = []
        self.box_display_radioframe = Tkinter.Frame(
            self.grid_page_right_side.interior())
        ...
        self.box_display_cylinder_size_frame = Tkinter.Frame(
            self.box_display_cylinder_frame.interior())
        self.box_display_cylinder_size_label = Label(
            self.box_display_cylinder_size_frame, text='Size:', width=10)
        ...
        self.box_display_mesh_line_width_frame = Tkinter.Frame(
            self.box_display_wire_frame.interior())
        self.box_display_mesh_line_width_label = Label(
            self.box_display_mesh_line_width_frame, text='Line Width:', width=10)
        ...
        self.box_display_mesh_grid_frame = Tkinter.Frame(
            self.box_display_wire_frame.interior())
        self.box_display_mesh_grid_label = Label(
            self.box_display_mesh_grid_frame, text='Grid Size:', width=10)
        ...
        self.box_display_radioframe.pack(
            padx=6, pady=6, expand='yes', fill='both')
        ...
        self.grid_center_radio_button_pymol_selection.pack(
            fill='x', expand=1, side=TOP)

        self.grid_center_radiogroups.append(
            self.grid_center_radio_button_pymol_selection)
        ...
        self.grid_center_selection_entry = Pmw.EntryField(self.grid_center_radio_button_pymol_selection.interior(),
                                                          ...
                                                          value=default_settings[
                                                              'grid_center_selection'],
                                                          command=self.grid_center_from_selection_changed
                                                          )
        self.grid_center_selection_entry.pack(
            fill='x', padx=4, pady=1, expand=0)
        ...
        self.grid_center_radio_button_coordinates.pack(
            fill='x', expand=1, side=TOP)

        self.grid_center_radiogroups.append(
            self.grid_center_radio_button_coordinates)

        self.grid_center_radioframe.pack(
            padx=6, pady=6, expand='yes', fill='both')
        Pmw.aligngrouptags(self.grid_center_radiogroups)

        self.grid_center_X_frame = Tkinter.Frame(
            self.grid_center_radio_button_coordinates.interior())
        self.grid_center_X_label = Label(self.grid_center_X_frame, text='X:')
        self.grid_center_X_location = Entry(
            self.grid_center_X_frame, textvariable=self.grid_center[0], bg='black', fg='green', width=10)
        self.grid_center_X_scrollbar = Scrollbar(
            self.grid_center_X_frame, orient='horizontal', command=self.grid_center_X_changed)

        self.grid_center_Y_frame = Tkinter.Frame(
            self.grid_center_radio_button_coordinates.interior())
        self.grid_center_Y_label = Label(self.grid_center_Y_frame, text='Y:')
        self.grid_center_Y_location = Entry(
            self.grid_center_Y_frame, textvariable=self.grid_center[1], bg='black', fg='green', width=10)
        self.grid_center_Y_scrollbar = Scrollbar(
            self.grid_center_Y_frame, orient='horizontal', command=self.grid_center_Y_changed)

        self.grid_center_Z_frame = Tkinter.Frame(
            self.grid_center_radio_button_coordinates.interior())
        self.grid_center_Z_label = Label(self.grid_center_Z_frame, text='Z:')
        self.grid_center_Z_location = Entry(
            self.grid_center_Z_frame, textvariable=self.grid_center[2], bg='black', fg='green', width=10)
        self.grid_center_Z_scrollbar = Scrollbar(
            self.grid_center_Z_frame, orient='horizontal', command=self.grid_center_Z_changed)
        ...
        self.select_binding_site_button_box = Pmw.ButtonBox(
            self.grid_center_radio_button_coordinates.interior(), orient='horizontal', padx=0, pady=0)
        self.select_binding_site_button_box.add(
            'Select binding site', command=self.select_atoms_within_binding_site)
        self.select_binding_site_button_box.pack(
            side=TOP, expand=1, padx=3, pady=3)

        # load/write gpf

        self.gpf_file_io = Pmw.Group(
            self.grid_definition_page, tag_text='GPF File')
        self.gpf_file_io.pack(side=TOP, expand=1, fill='x')

        self.gpf_file_location = Pmw.EntryField(self.gpf_file_io.interior(),
                                                labelpos='w',
                                                label_pyclass=FileDialogButtonClassFactory.get(
                                                    self.set_gpf_filename, mode='w', filter=[("Grid Parameter File", "*.gpf")]),
                                                validate={
                                                    'validator': quickFileValidation, },
                                                value=default_settings[
                                                    'gpf_file_name'],
                                                label_text='Autodock GPF File:')
        self.gpf_file_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=5)

        self.gpf_button_box = Pmw.ButtonBox(
            self.gpf_file_io.interior(), orient='horizontal', padx=0, pady=0)
        ...
        self.config_file_io = Pmw.Group(
            self.grid_definition_page, tag_text='Config File')
        self.config_file_io.pack(side=TOP, expand=1, fill='x')

        self.config_file_location = Pmw.EntryField(self.config_file_io.interior(),
                                                   labelpos='w',
                                                   label_pyclass=FileDialogButtonClassFactory.get(
                                                       self.set_config_filename, mode='w', filter=[("Vina Config File", "*.txt")]),
                                                   validate={
                                                       'validator': quickFileValidation, },
                                                   value=default_settings[
                                                       'config_file_name'],
                                                   label_text='VINA config File:')
        self.config_file_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=5)

        self.config_button_box = Pmw.ButtonBox(
            self.config_file_io.interior(), padx=0, pady=0, orient='horizontal')
        ...
        self.configuration_top_group = Pmw.Group(
            self.configuration_page, tag_text='General Notes')
        self.configuration_top_group.pack(
        ...
        self.configuration_group = Pmw.Group(
            self.configuration_page, tag_text='Scripts and Program Paths')
        ...
        self.autodock_tools_location = Pmw.EntryField(self.configuration_group.interior(),
                                                      labelpos='w',
                                                      label_pyclass=DirDialogButtonClassFactory.get(
                                                          self.set_autodock_tools_path),
                                                      value=self.config_settings[
                                                          'autodock_tools_path'],
        ...
        self.config_button_box = Pmw.ButtonBox(
            self.configuration_page, padx=0, pady=0, orient='horizontal')
        self.config_button_box.pack(side=BOTTOM, expand=1, padx=10, pady=0)
        self.config_button_box.add(
            'Save Plugin Configuration File', command=self.save_plugin_config_file)

        #------------------------------------------------------------------
        #
        #     RECEPTOR PREPARATION PAGE

        self.receptor_preparation_top_group = Pmw.Group(
            self.receptor_preparation_page, tag_text='Selections')
        self.receptor_preparation_top_group.pack(
            fill='both', expand=0, padx=10, pady=5)

        self.receptor_import_selection_button_box = Pmw.ButtonBox(
            self.receptor_preparation_top_group.interior(), orient='vertical', padx=0, pady=0)
        self.receptor_import_selection_button_box.add(
            'Import Selections', command=self.import_selections)
        self.receptor_import_selection_button_box.pack(
            side=LEFT, fill='x', padx=0, pady=3)

        self.receptor_pdbqt_location = Pmw.EntryField(self.receptor_preparation_top_group.interior(),
                                                      labelpos='w',
                                                      label_pyclass=FileDialogButtonClassFactory.get(
                                                          self.set_receptor_pdbqt_location, filter=[("PDBQT File", "*.pdbqt")]),

                                                      validate={
                                                          'validator': quickFileValidation, },
                                                      value='',
                                                      label_text='Receptor:')
        self.receptor_pdbqt_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=5)

        self.receptor_button_box = Pmw.ButtonBox(
            self.receptor_preparation_top_group.interior(), padx=0, pady=0, orient='horizontal')
        ...
        self.receptor_preparation_center_group = Pmw.Group(
            self.receptor_preparation_page, tag_text='Receptor Preparation')
        self.receptor_preparation_center_group.pack(
            fill='both', expand=0, padx=10, pady=5)

        self.selection_list = Pmw.ComboBox(self.receptor_preparation_center_group.interior(),
                                           scrolledlist_items=cmd.get_names(
                                               "selections") + cmd.get_names(),
                                           labelpos='nw',
        ...
        self.receptor_conversion_button_box = Pmw.ButtonBox(
            self.receptor_preparation_center_group.interior(), orient='vertical', padx=0, pady=0)
        self.receptor_conversion_button_box.add(
            'Generate Receptor ->', command=self.generate_receptor)
        self.receptor_conversion_button_box.add(
            'Select as Flexible ->', command=self.select_flexible_residues)
        self.receptor_conversion_button_box.add(
            'Remove Receptor', command=self.remove_receptor)
        self.receptor_conversion_button_box.add(
            'Remove Flexible', command=self.remove_flexible_residues)
        self.receptor_conversion_button_box.add(
            'Remove All', command=self.remove_all_receptors)
        self.receptor_conversion_button_box.pack(
            side=LEFT, expand=0, padx=0, pady=12, anchor='n')
        self.receptor_list.pack(side=LEFT, padx=3, anchor='n')
        self.flexible_residues_list.pack(side=LEFT, padx=3, anchor='n')

        self.receptor_preparation_bottom_group = Pmw.Group(
            self.receptor_preparation_page, tag_text='Log')
        self.receptor_preparation_bottom_group.pack(
            fill='both', expand=0, padx=10, pady=5)
        ...
        self.ligand_preparation_top_group = Pmw.Group(
            self.ligand_preparation_page, tag_text='Selections')
        self.ligand_preparation_top_group.pack(
            fill='both', expand=0, padx=10, pady=5)

        self.ligand_import_selection_button_box = Pmw.ButtonBox(
            self.ligand_preparation_top_group.interior(), orient='vertical', padx=0, pady=0)
        self.ligand_import_selection_button_box.add(
            'Import Selections', command=self.import_selections)
        self.ligand_import_selection_button_box.pack(
            side=LEFT, fill='x', padx=0, pady=3)

        self.ligand_dir_location = Pmw.EntryField(self.ligand_preparation_top_group.interior(),
                                                  labelpos='w',
                                                  label_pyclass=DirDialogButtonClassFactory.get(
                                                      self.set_ligand_dir_location),
                                                  validate={
                                                      'validator': quickFileValidation, },
                                                  value='',
                                                  label_text='Ligands:')
        self.ligand_dir_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=5)

        self.ligand_button_box = Pmw.ButtonBox(
            self.ligand_preparation_top_group.interior(), padx=0, pady=0, orient='horizontal')
        ...
        self.ligand_preparation_center_group = Pmw.Group(
            self.ligand_preparation_page, tag_text='Ligand Preparation')
        self.ligand_preparation_center_group.pack(
            fill='both', expand=0, padx=10, pady=5)
        ...
        self.ligand_conversion_button_box = Pmw.ButtonBox(
            self.ligand_preparation_center_group.interior(), orient='vertical', padx=0, pady=0)
        self.ligand_conversion_button_box.add(
            'Generate Ligand ->', command=self.generate_ligand)
        self.ligand_conversion_button_box.add(
            'Remove Ligand', command=self.remove_ligand)
        self.ligand_conversion_button_box.add(
            'Display Ligand', command=self.display_ligand)
        self.ligand_conversion_button_box.add(
            'Generate All', command=self.generate_all_ligands)
        self.ligand_conversion_button_box.add(
            'Remove All', command=self.remove_all_ligands)
        self.ligand_conversion_button_box.pack(
            side=LEFT, expand=0, padx=0, pady=12, anchor='n')
        self.ligand_pdbqt_list.pack(side=LEFT, padx=3, anchor='n')

        self.ligand_preparation_bottom_group = Pmw.Group(
            self.ligand_preparation_page, tag_text='Log')
        self.ligand_preparation_bottom_group.pack(
            fill='both', expand=0, padx=10, pady=5)
        ...
        self.docking_top_group = Pmw.Group(
            self.docking_page, tag_text='Docking')
        ...
        self.docking_center_group = Pmw.Group(
            self.docking_page, tag_text='AutoDock')
        self.docking_center_group.pack(fill='both', expand=0, padx=10, pady=5)
        self.docking_center_group2 = Pmw.Group(
            self.docking_page, tag_text='VINA')
        self.docking_center_group2.pack(fill='both', expand=0, padx=10, pady=5)

        self.docking_button_box = Pmw.ButtonBox(
            self.docking_center_group.interior(), orient='horizontal', padx=0, pady=0)
        ...
        self.docking_button_box.add(
            'Write AutoDock Input File(s)', command=self.write_autodock_input_files)

        self.docking_button_box2 = Pmw.ButtonBox(
            self.docking_center_group2.interior(), orient='horizontal', padx=0, pady=0)
        self.docking_button_box2.add('Run Vina', command=self.run_vina)
        self.docking_button_box2.add(
            'Write Vina Input File(s)', command=self.write_vina_input_files)
        ...
        self.docking_bottom_group = Pmw.Group(
            self.docking_page, tag_text='Log')
        ...
        self.pose_viewer_page_top_group = Pmw.Group(
            self.pose_viewer_page, tag_text='File')
        self.pose_viewer_page_top_group.pack(
            fill='both', expand=0, padx=10, pady=5)

        self.pose_viewer_page_stucts = Pmw.Group(
            self.pose_viewer_page, tag_text='Poses')
        self.pose_viewer_page_stucts.pack(
            fill='both', expand=1, padx=10, pady=0)

        self.pose_viewer_page_display = Pmw.Group(
            self.pose_viewer_page, tag_text='Display Options')
        self.pose_viewer_page_display.pack(fill='x', expand=1, padx=10, pady=0)

        self.pose_viewer_notebook = Pmw.NoteBook(
            self.pose_viewer_page_stucts.interior())
        ...
        self.pose_file_location = Pmw.EntryField(self.pose_viewer_page_top_group.interior(),
                                                 labelpos='w',
                                                 label_pyclass=FileDialogButtonClassFactory.get(
                                                     self.set_pose_filename, filter=[("PDBQT File", "*.pdbqt"), ("DLG File", "*.dlg")]),
                                                 validate={
                                                     'validator': quickFileValidation, },
                                                 value=default_settings[
                                                     'dlg_input_file'],
...
#        self.pose_file_location.pack(fill = 'both', expand = 1, padx = 10, pady = 5)
        self.pose_file_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=5)

        self.load_pose_file_buttonbox = Pmw.ButtonBox(
            self.pose_viewer_page_top_group.interior(), padx=0)
        self.load_pose_file_buttonbox.pack(
            side=BOTTOM, expand=1, padx=10, pady=5)
        self.load_pose_file_buttonbox.add(
            'Load', command=self.load_ligand_file)
        self.load_pose_file_buttonbox.add(
            'Load All', command=self.load_all_ligand_files)
        ...
        self.pose_viewer_ligand_display_radio.pack(
        ...
        self.rank_dat_file_io = Pmw.Group(
            self.rank_page, tag_text='Export scores as data file')
        ...
        self.rank_dat_file_location = Pmw.EntryField(self.rank_dat_file_io.interior(),
                                                     labelpos='w',
                                                     label_pyclass=FileDialogButtonClassFactory.get(
                                                         self.set_rank_dat_filename, mode='w', filter=[("Data File", "*.dat")]),
                                                     validate={
                                                         'validator': quickFileValidation, },
                                                     value=default_settings[
                                                         'rank_dat_file_name'],
                                                     label_text='Filename:')
        self.rank_dat_file_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=0)

        self.rank_dat_button_box = Pmw.ButtonBox(
            self.rank_dat_file_io.interior(), padx=0, pady=0, orient='horizontal')
        self.rank_dat_button_box.pack(side=BOTTOM, expand=1, padx=10, pady=0)
        self.rank_dat_button_box.add(
            'Export', command=self.export_score_dat_file)

        self.rank_csv_file_io = Pmw.Group(
            self.rank_page, tag_text='Export scores as CSV file')
        self.rank_csv_file_io.pack(side=TOP, expand=1, fill='x')

        self.rank_pose_file_io = Pmw.Group(
            self.rank_page, tag_text='Export poses as PDB file')
        ...
        self.rank_csv_file_location = Pmw.EntryField(self.rank_csv_file_io.interior(),
                                                     labelpos='w',
                                                     label_pyclass=FileDialogButtonClassFactory.get(
                                                         self.set_rank_csv_filename, mode='w', filter=[("CSV File", "*.csv")]),
                                                     validate={
                                                         'validator': quickFileValidation, },
                                                     value=default_settings[
                                                         'rank_csv_file_name'],
                                                     label_text='Filename:')
        self.rank_csv_file_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=0)

        self.rank_csv_button_box = Pmw.ButtonBox(
            self.rank_csv_file_io.interior(), padx=0, pady=0, orient='horizontal')
        self.rank_csv_button_box.pack(side=BOTTOM, expand=1, padx=10, pady=0)
        self.rank_csv_button_box.add(
            'Export', command=self.export_score_csv_file)

        self.rank_pose_file_location = Pmw.EntryField(self.rank_pose_file_io.interior(),
                                                      labelpos='w',
                                                      label_pyclass=FileDialogButtonClassFactory.get(
                                                          self.set_rank_pose_filename, mode='w', filter=[("PDB File", "*.pdb")]),
                                                      validate={
                                                          'validator': quickFileValidation, },
                                                      value=default_settings[
                                                          'rank_pose_file_name'],
                                                      label_text='Filename:')
        self.rank_pose_file_location.pack(
            side=LEFT, fill='x', expand=1, padx=1, pady=0)

        self.rank_pose_button_box = Pmw.ButtonBox(
            self.rank_pose_file_io.interior(), padx=0, pady=0, orient='horizontal')
        self.rank_pose_button_box.pack(side=BOTTOM, expand=1, padx=10, pady=0)
        self.rank_pose_button_box.add(
            'Export', command=self.export_score_pose_file)
        ...
        self.map_viewer_page_top_group = Pmw.Group(
            self.map_viewer_page, tag_text='Grid Map')
        self.map_viewer_page_top_group.pack(
            fill='both', expand=0, padx=10, pady=5)

        # the maps card

        self.map_viewer_page_center_group = Pmw.Group(
            self.map_viewer_page, tag_text='Maps')
        self.map_viewer_page_center_group.pack(
            fill='both', expand=1, padx=10, pady=5)
        self.map_viewer_notebook = Pmw.NoteBook(
            self.map_viewer_page_center_group.interior())
        ...
        self.map_file_location = Pmw.EntryField(self.map_viewer_page_top_group.interior(),
                                                labelpos='w',
                                                label_pyclass=FileDialogButtonClassFactory.get(
                                                    self.set_mapfilename, filter=[("Autodock Map File", "*.map")]),
                                                validate={
                                                    'validator': quickFileValidation, },
                                                value=default_settings[
                                                    'map_input_file'],
        ...
        self.map_file_location.pack(
            side=LEFT, fill='x', expand=1, padx=10, pady=5)

        self.load_map_buttonbox = Pmw.ButtonBox(
            self.map_viewer_page_top_group.interior(), padx=0)
        ...
        self.status_line.configure(
            text="Ready..... (version: %s)" % __version__)
        #------------------------------------------------------------------
        ##################################################################
    ...
    def calculate_grid_center(self):
        if self.grid_center_selection_mode.get() == GRID_CENTER_FROM_SELECTION:
            ...
            if sel:
                ...
                self.grid_center_selection_mode.set(
    ...
    def select_atoms_within_binding_site(self):
        ...
        if len(by_id) > 1:
            ...
            self.status_line.configure(
                text="Selector 'binding_site' created with %d atoms" % len(by_id))
    ...
    def save_gpf_file(self):
        ...
        print >>fp, 'gridcenter  %8.3f %8.3f %8.3f' % (
    ...
    def read_plugin_config_file(self):
        ...
        if os.path.isfile(config_file_name):
            self.status_line.configure(
                text='Reading configuration file: %s' % config_file_name)
    ...
    def save_plugin_config_file(self):
        ...
        self.config_settings[
            'autogrid_exe'] = self.autogrid_location.getvalue()
        self.config_settings[
            'autodock_exe'] = self.autodock_location.getvalue()
        self.config_settings['vina_exe'] = self.vina_location.getvalue()
        self.config_settings[
            'autodock_tools_path'] = self.autodock_tools_location.getvalue()
        print 'ADDD', self.autodock_location.getvalue()
        for key, val in self.config_settings.items():
            print >>fp, key, '=', val
        fp.close()
        self.status_line.configure(
            text='Wrote configuration file %s' % config_file_name)
...
# print 'Selection:', sels[0]
    ...
    def generate_receptor(self):
        ...
        util_program = os.path.join(
            self.autodock_tools_path.get(), "prepare_receptor4.py")
        outfilename = os.path.join(
            self.work_dir(), "receptor.%s.pdbqt" % sel[0])
        command = "%s -r %s -o %s -A checkhydrogens" % (
            util_program, tmp_rec_pdb, outfilename)
        ...
        if result == 0:
            ...
            self.status_line.configure(
                text="Successfully generated receptor file receptor.%s.pdbqt" % sel[0])
            ...
            stored.list = []
            cmd.iterate(
                sel[0] + ' and name ca', "stored.list.append([resi,resn])")
            ...
            self.status_line.configure(
                text="An error occured while preparing receptor from selection %s" % sel[0])
            self.receptor_page_log_text.insert('end', output)
    ...
    def select_flexible_residues(self):
        ...
        stored.list = []
        cmd.iterate(
            sel + ' and name ca', "stored.list.append([chain, resn,resi])")
...
#                                   % (len(receptor_object.flexible_residues), rec))
        util_program = os.path.join(
            self.autodock_tools_path.get(), "prepare_flexreceptor4.py")
        ...
        command = "%s -r %s -s %s -g %s -x %s" % \
                  (util_program, receptor_filename,
                   flex_res_string, rec_rigid, rec_flexible)
        self.receptor_page_log_text.insert('end', "Batch: %s\n" % command)
        self.receptor_page_log_text.yview('moveto', 1.0)
#        result = os.system(command)
        ...
        if result == 0:
            ...
            self.status_line.configure(
                text="Successfully generated flexible receptor files for %s(%s)" % (rec, sel))
#            self.receptor_page_log_text.insert('end',"Successfully generated flexible receptor files for %s(%s)\n" % (rec,sel))
            ...
            self.status_line.configure(
                text="An error occured while preparing a flexible receptor from selection %s" % sel)
            self.receptor_page_log_text.insert('end', output)
        self.receptor_page_log_text.yview('moveto', 1.0)
    ...
    def load_receptor_pdbqt(self):
        ...
        outfile = os.path.join(
            self.work_dir(), os.path.basename(fn).split('.')[0] + '_pdb.pdb')
        util_program = os.path.join(
            self.autodock_tools_path.get(), "pdbqt_to_pdb.py")
        ...
        if result == 0:
            ...
            stored.list = []
            cmd.iterate(
                name + ' and name ca', "stored.list.append([resi,resn])")
            for resi, resn in stored.list:
                r.resi_dic[int(resi)] = resn
            ...
            self.status_line.configure(
                text="An error occured while loading receptor  %s" % fn)
            self.receptor_page_log_text.insert('end', output)
    ...
    def remove_flexible_residues(self):
        ...
        self.status_line.configure(
            text="Removed flexible residues from receptor %s" % rec)
        self.selected_receptor(rec)
    ...
    def save_as_ligand_pdb(self, name):
        ...
        self.status_line.configure(
            text="Saving ligand pdb file %s from selection %s" % (pdb_name, name))
    ...
    def generate_ligand(self):
        ...
        outfile = os.path.join(
            self.work_dir(), os.path.basename(filename).split('.')[0] + '.pdbqt')
        util_program = os.path.join(
            self.autodock_tools_path.get(), "prepare_ligand4.py")
        command = "%s -l %s -o %s -A checkhydrogens" % (
            util_program, filename, outfile)
...
#        result = os.system(command)
        ...
        if result == 0:
            ...
            self.status_line.configure(
                text="Successfully generated ligand file %s" % outfile)
            ...
            self.status_line.configure(
                text="An error occured while preparing ligand file from %s" % sel)
            self.ligand_page_log_text.insert('end', output)
    ...
    def import_ligands(self):
        ...
        for f in lst:
            ...
            self.ligand_page_log_text.insert(
                'end', "Importing ligand  %s\n" % f)
            self.ligand_page_log_text.yview('moveto', 1.0)
            self.ligand_list.insert('end', name)
    ...
    def run_autogrid(self):
        ...
        if use_flex == 'No':
            ...
            if receptor_object.receptor_flexible:
                ...
                self.status_line.configure(
                    text="No flexible receptor defined! Cannot do flexible docking!")
                return
        self.docking_page_log_text.insert(
            'end', '#---------------------------------------\n')
        self.docking_page_log_text.insert(
            'end', '# SETTING UP GRID CALCULATION\n')
        self.docking_page_log_text.insert(
            'end', ' > RECEPTOR RIGID    : %s\n' % rigid_receptor_file)
        self.docking_page_log_text.insert(
            'end', ' > RECEPTOR FLEXIBLE : %s\n' % flex_receptor_file)
        ...
        self.docking_page_log_text.insert(
            'end', ' > GRID POINTS      : %d %d %d\n' % (n_points_X, n_points_Y, n_points_Z))
        self.docking_page_log_text.insert(
            'end', ' > GRID CENTER      : %8.3f %8.3f %8.3f\n' % (center_X, center_Y, center_Z))
        self.docking_page_log_text.insert(
            'end', ' > GRID SPACING     : %8.3f \n' % spacing)
        ...
        fp.close()
        util_program = os.path.join(
            self.autodock_tools_path.get(), "prepare_gpf4.py")
        command = "%s -r %s -i %s -o %s" % (util_program,
                                            rigid_receptor_file, template_gpf, outfile_gpf)
        ...
        if result == 0:
            self.status_line.configure(
                text="Successfully generated AutoGrid input file")
            ...
            self.status_line.configure(
                text="An error occured while trying to run Autogrid....")
            self.docking_page_log_text.insert('end', output)
        self.docking_page_log_text.yview('moveto', 1.0)
    ...
    def run_autodock(self, write_only=False):
        ...
        util_program = os.path.join(
            self.autodock_tools_path.get(), "prepare_dpf4.py")
        if ligands != 'All':
            outfile_dpf = ligands + '.dpf'
            command = "%s -r %s -i %s -o %s " % (
                util_program, rigid_receptor_file, template_dpf, outfile_dpf)
            if flex_receptor_file is not None:
                command += ' -x %s' % flex_receptor_file
            else:
                command += ' -l %s' % lig_pdbqt
            ...
            if result == 0:
                self.status_line.configure(
                    text="Wrote AutoDock input file for ligand: %s" % ligands)
#                self.docking_page_log_text.insert('end',"Running AutoDock.....\n")
                ...
                command = '%s -p %s -l %s' % (autodock,
                                              outfile_dpf, outfile_poses)
#                self.status_line.configure(text="Now docking ligand: %s...." % ligands)
                ...
                r = Thread_run(
                    command, self.current_thread, self.status_line, "Now docking ligand: %s...." % ligands)
                r.start()
                ...
                self.status_line.configure(
                    text="Error while generating run input file for ligand: %s" % ligands)
                self.docking_page_log_text.insert('end', output)
                self.docking_page_log_text.yview('moveto', 1.0)
    ...
    def run_vina(self, write_only=False):
        ...
        if ligands != 'All':
            outfile_conf = os.path.join(
                self.work_dir(), ligands + '.vina_config.txt')
            outfile_log = os.path.join(self.work_dir(), ligands + '.vina.log')
            outfile_poses = os.path.join(
                self.work_dir(), ligands + '.docked.pdbqt')
            self.ligand_dic[ligands].outfile_poses = outfile_poses
            ...
            fp.close()
            self.status_line.configure(
                text="Wrote VINA input file for ligand: %s" % ligands)
...
#            self.status_line.configure(text="Now docking ligand: %s...." % ligands)
            if os.path.isfile(outfile_log):
                shutil.move(outfile_log, outfile_log + '~')
            os.system('touch %s' % outfile_log)
            r = Thread_run(command, self.current_thread,
                           self.status_line, "Now docking ligand: %s...." % ligands)
            r.start()
            self.current_thread = r
            ll = Thread_log(outfile_log, self.docking_page_log_text)
            ll.start()
    ...
    def load_dlg(self, filename):
        ...
        for i in range(len(pose_list)):
            ...
            self.pose_viewer_ligand_dic[name].update(
                {name + '::%d' % (i + 1): pose_list[i]})
...
#            print model.energy
    ...
    def update_combo(self, name):
        try:
            ...
            pass
        self.pose_viewer_ligand_pages[name] = {
            'name': self.pose_viewer_notebook.add(name)}
        pose_list = self.pose_viewer_ligand_dic[name].keys()

        pose_list.sort(
            lambda a, b: cmp(int(a.split('::')[1]), int(b.split('::')[1])))
        self.pose_viewer_ligand_pages[name].update({'poses': pose_list})

        self.pose_viewer_buttonbox = Pmw.ButtonBox(
            self.pose_viewer_ligand_pages[name]['name'], padx=3)
        self.pose_viewer_buttonbox.add(
            'Show best 10', command=self.show_best_poses)
        ...
        self.pose_viewer_ligand_pages[name]['combo'] = Pmw.ComboBox(self.pose_viewer_ligand_pages[name]['name'],
                                                                    ...
                                                                    dropdown=False)
        self.pose_viewer_ligand_pages[name][
            'combo'].pack(side='left', padx=3, anchor='n')
    ...
    def ligand_combo_box_selected(self, value):
        ...
        if self.pose_viewer_radiobuttons.getvalue() == 'Show Selected':
            ...
            self.pose_viewer_ligand_pages[name][
                'text'].insert('end', ligand.info_string())
        else:
            pmlname = value.replace(' ', '_').replace('::', '_')
            cmd.delete(pmlname)
            self.pose_viewer_ligand_pages[name]['text'].clear()
...
#           cmd.delete(pmlname+'.*')
    ...
    def export_score_dat_file(self):
        ...
        self.status_line.configure(
            text="Exported docking results to %s" % filename)
    ...
    def export_score_pose_file(self):
        ...
        for i, ligand in enumerate(ligand_list):
            ...
            print >>fp, "ENDMDL"
        self.status_line.configure(
            text="Exported docking poses to %s" % filename)
...
#        self.map_pages[name].update({'maps':self.map_dic.keys()})
        ...
        self.map_thresholdloc = Entry(self.map_thresholdfr, textvariable=self.map_threshold[
                                      name], bg='black', fg='green', width=15)
        self.scrmap_threshold = Scrollbar(
            self.map_thresholdfr, orient="horizontal", command=self.change_map_threshold)

        labmap_threshold.pack(side=LEFT)
        self.map_thresholdloc.pack(side=LEFT)
        self.scrmap_threshold.pack(side=LEFT)
        self.map_thresholdfr.pack(
            fill='x', padx=4, pady=3, side=LEFT)  # vertical
        ...
        self.map_pages[name]['combo'] = Pmw.ComboBox(self.lower_part,
                                                     ...
                                                     labelpos='nw',
                                                     scrolledlist_items=self.map_dic[
                                                         name].keys(),
                                                     selectioncommand=self.status_mapbox,
                                                     listbox_height=15,
                                                     listbox_width=1,
                                                     dropdown=False)
        self.map_pages[name]['combo'].pack(
            side=LEFT, padx=3, anchor='n', fill='y')
    ...
    def fileopen(self, filename, mode):
        if mode == 'w' and os.path.isfile(filename):
            ...
            self.status_line.configure(
                text='Backing up %s to %s' % (filename, fn))
        try:
...
class PmwDirDialog(PmwFileDialog):
    ...
    def __init__(self, parent=None, **kw):
        ...
        # Create the directory list component.
        dnb = self.mkdnb()
        dnb.grid(row=0 + rowoffset, column=0, columnspan=2,
        ...
        for row in xrange(0, rows):
            ...
            for colx in xrange(0, cols):
                self.entry[row][colx] = TableCell(self.table, row=row,
                                                  ...
                                                  command=self)
                self.entry[row][colx].grid(
                    row=row + 1, column=colx, sticky=N + W + S + E)
        ...
        if self.lastVisible < len(self.data.values()):
            for row in xrange(0, self.windowSize):
                for col in xrange(0, self.cols):
                    try:
                        self.entry[row][col]["text"] = self.data[
                            row + firstVisible][col]
                    except:
                        pass
